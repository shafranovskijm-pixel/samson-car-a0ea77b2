...
var upsertServiceByCategoryName = async (input) => {
	const { data: existing, error: findErr } = await supabase.from("services").select("*").ilike("category", input.category).ilike("name", input.name).is("deleted_at", null).maybeSingle();
...
var listPaymentsRange = async (from, to) => {
	const { data, error } = await anySb.from("appointment_payments").select("*").gte("paid_at", from).lte("paid_at", to).order("paid_at", { ascending: false });
	if (error) throw error;
	return data ?? [];
};
//#endregion
export { updateBrand as $, humanizeSupabaseError as A, listClients as B, deleteMechanic as C, deleteServicePrice as D, deleteService as E, listAppointmentsByClient as F, listMechanicShifts as G, listMechanicAdvances as H, listBrands as I, listPricesForBrand as J, listMechanics as K, listCarModels as L, listAllMechanicShifts as M, listAppointmentPayments as N, getAppointment as O, listAppointments as P, updateAppointmentStatus as Q, listCars as R, deleteExpense as S, deleteMechanicShift as T, listMechanicPayouts as U, listExpenses as V, listMechanicServiceRates as W, listServices as X, listServicePrices as Y, updateAppointment as Z, deleteBrand as _, createBrand as a, updateMechanicDefaultPayoutPercent as at, deleteClient as b, createClient as c, upsertMechanicServiceRate as ct, createMechanic as d, updateCar as et, createMechanicAdvance as f, deleteAppointmentPayment as g, deleteAppointment as h, createAppointmentPayment as i, updateMechanic as it, listAllClientComments as j, getPriceForBrand as k, createClientComment as l, upsertServiceByCategoryName as lt, createService as m, clearAppointmentPayments as n, updateClient as nt, createCar as o, updateMechanicShift as ot, createMechanicShift as p, listPaymentsRange as q, createAppointment as r, updateClientComment as rt, createCarModel as s, updateService as st, api_exports as t, updateCarModel as tt, createExpense as u, upsertServicePrice as ut, deleteCar as v, deleteMechanicAdvance as w, deleteClientComment as x, deleteCarModel as y, listClientComments as z };
