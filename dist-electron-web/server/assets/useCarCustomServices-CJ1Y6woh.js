...
var useCarCustomServices = (brand, model, year) => {
	...
	return {
		...
		add: useCallback(async (input) => {
			...
			const existing = items.find((x) => x.name.trim().toLowerCase() === nameNorm && x.category.trim().toLowerCase() === catNorm);
			const payload = {
				brand_name: brand,
				model_name: model,
				year,
			...
			const { data, error } = await supabase.from("car_custom_services").upsert(payload, {
